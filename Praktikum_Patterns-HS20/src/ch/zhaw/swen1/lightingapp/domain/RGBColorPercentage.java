...
public class RGBColorPercentage {
	...
	/**
	 * Returns true if all percentages are below the specified level. 
	 * @param percentages
	 * @param level
	 * @return
	 */
	public static boolean isLess(Iterable<RGBColorPercentage> percentages, RGBColorPercentage level){
		for(RGBColorPercentage percentage : percentages){
			if (!percentage.isPercentageBelow(level)){
				return false;
			}
		}
		return true;
